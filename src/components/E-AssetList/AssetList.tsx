...
interface AssetListProps {
  ...
  onExportGLBDataAsJSON: () => void;
}

export default function AssetList({ assets, onDeleteAsset, onExportGLBDataAsJSON }: AssetListProps) {
  ...
  return (
    <div className="bg-white border-l border-gray-200 h-full flex flex-col">
      ...
      <div className="p-4 border-t border-gray-200">
        ...
        <button
          onClick={onExportGLBDataAsJSON}
          disabled={assets.filter(asset => asset.fileType === 'glb').length === 0}
          className={`w-full py-2 px-4 rounded-lg font-medium text-sm transition-colors mb-3 ${
            assets.filter(asset => asset.fileType === 'glb').length === 0
        ...
        >
          <div className="flex items-center justify-center space-x-2">
            ...
            <span>Export GLB Data as JSON</span>
          </div>
        </button>
